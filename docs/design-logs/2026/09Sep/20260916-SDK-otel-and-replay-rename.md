...
**Goals**
1. One **opt-in** parent CLIENT span per `execute`/`execute_async`/`steps`/`events`, emitted into the caller's OTel provider, following OTel semantic conventions (span kind CLIENT; `gen_ai.*` reserved for the deferred per-step spans).
...
Each `execute`/`execute_async`/`steps`/`events` call is wrapped in one span:

| Aspect | Value |
|---|---|
| **Name** | `noukai.flow.execute`, `noukai.flow.execute_async`, `noukai.flow.steps`, `noukai.flow.events` |
| **Kind** | `CLIENT` |
| **Attributes (set at start)** | `noukai.org`, `noukai.project`, `noukai.flow.slug`, `noukai.flow.version` |
| **Attributes (set at end)** | `noukai.execution_id` (when known), `noukai.flow.status` (`completed`/`failed`/`tool_calls_required`/`accepted`) |
| **Status** | `OK` on success; on exception → `record_exception(e)` + `set_status(ERROR)` then re-raise |
| **Lifetime** | `execute`/`execute_async`: around the request. `steps`/`events`: around the whole iterator (opened when iteration starts, closed when the generator is exhausted or errors) so duration reflects the full step-through, not just the first request. |

The wrap is a thin `with span_factory.flow_span("execute", org, project, slug, version) as span:` around the existing body; the no-op factory makes this a plain pass-through when off. Sync and async both get it (Python needs it in both `Flow` and `AsyncFlow`; the iterator wrap lives in `_step_iterator.py`).
...
| Flow | wrap `execute`/`execute_async` bodies in `flow_span(...)` (sync + async) | wrap `execute`/`executeAsync` |
| Step iterator | wrap the `steps`/`events` iterator lifetime in a span | mirror in `step-iterator.ts` |
| Errors | `NoukaiError` subclass or clear message when `otel=True` but extra missing | thrown `Error`/`NoukaiError` when peer dep absent |
...
**Deferred (PR-C, documented, not built):** per-step child spans from `run.trace()` (backdated `start_time`/`end_time` from `StepTrace.started_at`/`completed_at`; attributes `gen_ai.request.model`=`model_used`, `gen_ai.usage.input_tokens`/`output_tokens`=`tokens.prompt`/`completion`, `noukai.step.cost_usd`=`cost_usd`, `noukai.step.id`/`status`); and `traceparent` injection via `extra_headers` behind a `propagate_context=True` flag.
...
**PR-B (OTel):**
- **Off by default:** with no `otel=`, a fake that fails if `opentelemetry` is imported proves the no-op path never touches OTel; execute/steps/events behave identically to today.
- **On:** with an in-memory span exporter (Py `InMemorySpanExporter`; TS a fake tracer/exporter), one CLIENT span per call with the expected name + attributes; `noukai.execution_id`/`status` set on completion.
- **Error path:** an execute that raises records the exception and sets span status ERROR, and the exception still propagates unchanged.
- **`steps`/`events`:** span spans the whole iteration.
- **Missing extra/peer dep:** `otel=True` without the dependency raises the crisp install-hint error.
...
- **R4 — `steps`/`events` span lifetime.** Wrapping a lazy iterator means the span must open at first iteration and close on exhaustion/error, not at method-return. *Mitigation:* explicit start/finish in the iterator wrapper with a `finally`; tested for early-break and error.
...
2. **OTel v1 = parent CLIENT span only.** Highest value (end-to-end latency + status in the caller's stack), lowest risk, no extra network. Per-step spans + `traceparent` are designed but deferred to PR-C.
